...
int IcmpTest_Send(int argc, char *argv[]);
int IcmpTest_Recv(int argc, char *argv[])
...
int IcmpTest(int argc, char *argv[])
{
	...
	ret = IcmpTest_Send(argc, argv);
//	ret = IcmpTest_Recv(argc, argv);
...
int IcmpTest_Send(int argc, char *argv[])
{
	if (5 != argc)
	{
		printf("usage:[localIP] [remoteIP] [packetNum] [intervalTimeUS]\n");
		exit(-1);
	}
...
void SendIcmpPacket(int sockFd, struct sockaddr_in *remoteAddr, int packetNum, int dataLen, int intervalTimeUS)
{
	int idx;
